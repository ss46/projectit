<template>
    <div class="sticky top-0 ">
        ...
        <div v-if="sessionResource.loadnig">
            Loading...
        </div>
        <div class="bg-gradient-to-r from-[#4A6BB6] to-[#7F9DE0] h-16 flex items-center justify-between"
            v-else-if="sessionResource.data.company">
            <div class=" pl-4 pr-3" @click="router.push({ name: 'Home' })">
                
            </div>
            <div class="text-[#e5e8ed] font-[700] text-[25px] font-[Inter]"> {{ sessionResource.data.company }} </div>
            <div class="pr-4">
                <div class="w-11"></div>
...
import { session, sessionResource } from '../data/session'
